...
Page({
  ...
  async fetchSchedule(forceRefresh = false) {
    ...
    try {
      ...
      if (currentDataSourceIndex === 1) {
        ...
        res = await api.getPortalSchedule(formatDate(monday), formatDate(sunday));
      } else {
        res = await api.getSchedule(selectedDate);
      }
  ...
  processCourses() {
    const { allCourses, currentWeek, periodHeight, currentDataSourceIndex } = this.data;
    ...
    const customCourses = useWeekBasedCustomCourses ? customCourseStorage.getByWeek(weekNum) : [];
    const mergedCourses = [
      ...allCourses,
